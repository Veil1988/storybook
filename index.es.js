import React__default, { useMemo, createElement, useState, useCallback, useEffect, useRef, Component, createContext, useLayoutEffect, useContext, forwardRef } from 'react';
import { IMaskInput, IMaskMixin } from 'react-imask';
import ne, { findDOMNode } from 'react-dom';
...
const Dots = (props) => {
    const { size = DotsSizeEnum.regular, className = '' } = props;
    // Memo
    const memoClassName = useMemo(() => `${DotsClassEnum[size]} ${className}`, [size, className]);
...
const Typography = ({ type = TypographyType.body, weight, value, children, className = '', margin, ellipsis, multilineEllipsis, tag = TypographyTag.p, font }) => {
...
const Button = (props) => {
    const { handleClick, value = '', isDisabled = false, isLoading = false, type = TypeButtonEnum.default, size = SizeButtonEnum.regular, className = '', } = props;
    // State //
    const [isAnimation, toggleAnimation] = useState(false);
    // Memo //
    const memoDisabled = useMemo(() => isDisabled, [isDisabled]);
    const memoLoading = useMemo(() => isLoading, [isLoading]);
    const memoTypographyType = useMemo(() => {
        if (size === SizeButtonEnum.regular) {
            return TypographyType.body;
        }
        if (size === SizeButtonEnum.small) {
            return TypographyType.small;
        }
        if (size === SizeButtonEnum.large) {
            return TypographyType.large;
        }
    }, [size]);
    const buttonClassName = useMemo(() => `${TypeButtonClassEnum[type]} ${SizeButtonClassEnum[size]} ${(isAnimation && 'animation-Click') || ''} ${className}`, [type, size, className, isAnimation]);
    // Callbacks //
    const useCallbackHandleClick = useCallback((event) => {
    ...
    return (React__default.createElement("button", { type: "button", className: buttonClassName, disabled: memoDisabled, onClick: useCallbackHandleClick },
        memoLoading && React__default.createElement(Dots, { size: DotsSizeEnum[size] }),
...
const Link = (props) => {
    const { value, type = TypeLinkEnum.regular, className = '' } = props;
    // Memo
    const memoValue = useMemo(() => value, [value]);
    const memoIsExternalLink = useMemo(() => type === TypeLinkEnum.external, [type]);
    const memoClassName = useMemo(() => `uiKit-link-wrapper ${className}`, [className]);
...
function Checkbox({ name, isActive, hasError = false, disabled = false, type = TypeCheckboxEnum.normal, onClick, onChange, onChangeField, className = '', }) {
    // Memo
...
const H1 = (props) => {
    ...
    const memoValue = useMemo(() => value || '', [value]);
    const memoClassName = useMemo(() => `uiKit-typography-h1${(maxWidth && '_eclipses') || ''} ${className}`, [className, maxWidth]);
...
const H2 = (props) => {
    const { value, children, className = '', margin, maxWidth } = props;
    const memoValue = useMemo(() => value || '', [value]);
    const memoClassName = useMemo(() => `uiKit-typography-h2${(maxWidth && '_eclipses') || ''} ${className}`, [className, maxWidth]);
    const style = { margin: `${margin}`, maxWidth: (maxWidth && `${maxWidth}px`) || 'auto' };
...
const H3 = (props) => {
    const { value, children, className = '', margin, maxWidth } = props;
    const memoValue = useMemo(() => value || '', [value]);
    const memoClassName = useMemo(() => `uiKit-typography-h3${(maxWidth && '_eclipses') || ''} ${className}`, [className, maxWidth]);
    const style = { margin: `${margin}`, maxWidth: (maxWidth && `${maxWidth}px`) || 'auto' };
...
const H4 = (props) => {
    const { value, children, className = '', margin, maxWidth } = props;
    const memoValue = useMemo(() => value || '', [value]);
    const memoClassName = useMemo(() => `uiKit-typography-h4${(maxWidth && '_eclipses') || ''} ${className}`, [className, maxWidth]);
    const style = { margin: `${margin}`, maxWidth: (maxWidth && `${maxWidth}px`) || 'auto' };
...
const Large = (props) => {
    ...
    const memoValue = useMemo(() => value || '', [value]);
    const memoClassName = useMemo(() => `${LargeClassNameEnum[weight] || LargeClassNameEnum.regular}${(maxWidth && '_eclipses') || ''} ${className}`, [weight, maxWidth, className]);
    const style = { margin: `${margin}`, maxWidth: (maxWidth && `${maxWidth}px`) || 'auto' };
...
const Body = (props) => {
    ...
    const memoValue = useMemo(() => value || '', [value]);
    const memoClassName = useMemo(() => `${BodyClassNameEnum[weight] || BodyWeightEnum.regular}${(maxWidth && '_eclipses') || ''} ${className}`, [className, maxWidth, weight]);
    const style = { margin: `${margin}`, maxWidth: (maxWidth && `${maxWidth}px`) || 'auto' };
    return (React__default.createElement("p", { style: style, className: memoClassName },
        memoValue,
        children));
...
const Small = (props) => {
    ...
    const memoValue = useMemo(() => value || '', [value]);
    const memoClassName = useMemo(() => `${SmallClassNameEnum[weight] || SmallClassNameEnum.regular}${(maxWidth && '_eclipses') || ''} ${className}`, [weight, maxWidth, className]);
    const style = { margin: `${margin}`, maxWidth: (maxWidth && `${maxWidth}px`) || 'auto' };
    return (React__default.createElement("p", { style: style, className: memoClassName },
        memoValue,
        children));
...
})(TypeTypographyEnum || (TypeTypographyEnum = {}));
var WeightTypographyEnum;
...
const Typography$1 = (props) => {
    ...
    switch (type) {
        ...
        default:
            return (React__default.createElement(Body, { value: TypeTypographyEnum.body, weight: WeightTypographyEnum.regular, className: className, margin: margin, maxWidth: maxWidth }, children));
    }
};
...
})(TypeSimpleIconEnum || (TypeSimpleIconEnum = {}));
...
function Input({ className = '', placeholder, value = '', handleChange, hasError = false, hint = '', disabled = false, onPressEnter, onPressEsc, onPressArrow, onFocus, onBlur, name, onChange, onChangeField, label = '', validation = [], withMask = undefined, icon, iconSide = IconSideEnum.right, htmlFor = false, }) {
    ...
    const handleAccept = useCallback((val, mask, event) => {
    ...
    return (React__default.createElement("div", { className: memoClassName },
        ...
        React__default.createElement("label", { htmlFor: forId, className: "uiKit-input-label" },
            React__default.createElement(Typography$1, { value: label, type: TypeTypographyEnum.small, weight: WeightTypographyEnum.bold, margin: "0 0 4px 12px" }))),
        React__default.createElement("div", { className: "uiKit-field-wrapper" },
            (withMask && (React__default.createElement(IMaskInput, Object.assign({}, withMask, { 
                // @ts-ignore
            ...
            (icon && icon in TypeSimpleIconEnum && (React__default.createElement(SimpleIcon, { className: IconClassEnum[iconSide], type: icon }))) || React__default.createElement(Typography$1, { className: IconClassEnum[iconSide], value: icon })),
        hintMessage && (React__default.createElement("span", { className: "uiKit-input-hint" },
            React__default.createElement(Typography$1, { value: hintMessage, type: TypeTypographyEnum.small })))));
}
...
})(LabeSizeEnum || (LabeSizeEnum = {}));

const Label = (props) => {
    const { value, size = LabeSizeEnum.regular, type = LabelTypeEnum.default, className = '', } = props;
    // Memo
    const memoValue = useMemo(() => value, [value]);
    const memoClassName = useMemo(() => `${LabelClassEnum[type]}${(size === LabeSizeEnum.regular && '_regular') || '_small'} ${className}`, [type, size, className]);
    const memoTypographyType = useMemo(() => (size === LabeSizeEnum.regular && TypographyType.body) || TypographyType.small, [size]);
    return (React__default.createElement("div", { className: memoClassName },
...
var NotificationSizeEnum;
(function (NotificationSizeEnum) {
    NotificationSizeEnum["regular"] = "regular";
    NotificationSizeEnum["small"] = "small";
})(NotificationSizeEnum || (NotificationSizeEnum = {}));

const Notification = (props) => {
    const { id, value, size = NotificationSizeEnum.regular, type = NotificationTypeEnum.default, closable = true, className = '', closeCallback, } = props;
    // useState
    const [isVisible, toggleVisible] = useState(true);
    // useMemo
    const memoClassName = useMemo(() => `${NotificationClassEnum[type]}_${NotificationSizeEnum[size]} ${className}`, [type, size, className]);
    const TypographyTypeText = useMemo(() => (size === NotificationSizeEnum.regular && TypographyType.body)
        || TypographyType.small, [size]);
    // useCallback
...
})(AutoCloseNitificationType || (AutoCloseNitificationType = {}));
...
const AutoCloseNotification = (props) => {
    var _a;
    const { value, timeToClose = default_time, className = '', type = AutoCloseNitificationType.warning, id, closeCallback, } = props;
    // useRef
    ...
    // useState
    const [isVisible, toggleVisible] = useState(true);
    const [timeCount, changeTime] = useState(0);
    // useMemo
    const memoProgressBarClassName = useMemo(() => AutoCloseProgressClassEnum[type], [type]);
    const memoClassName = useMemo(() => `uiKit-autoCloseNotification-wrapper ${className}`, [className]);
    // useEffect
    useEffect(() => {
    ...
    return (React__default.createElement(React__default.Fragment, null, isVisible && (React__default.createElement("div", { ref: notificationRef, className: memoClassName, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseOut },
        React__default.createElement(Notification, { id: id, value: value, size: NotificationSizeEnum.regular, type: type, closeCallback: closeCallback }),
...
const NotificationsContainer = (props) => {
    const [notificationList, updateNotification] = useState([]);
    const { className = '' } = props;
    // Memo
    const memoClassName = useMemo(() => `uiKit-notificationsContainer ${className}`, [className]);
    // Effect
    useEffect(() => {
    ...
    // Callback
    const closeCallback = useCallback((id) => {
...
function Dropdown({ htmlFor, placeholder, value, handleSelect, onChange, onChangeField, hasError = false, disabled = false, options, name, className = '', disableFilter = false, label = '', ref, }) {
    ...
    // useState
    const [searchValue, changeSearchValue] = useState(findValue(value));
    const [isOpenList, changeIsOpenList] = useState(false);
    const [isNotChangeValue, toogleIsNotChangeValue] = useState(true);
    const [selectedValue, changeSelectedValue] = useState(findValue(value));
    const [arrowIndex, changeArrowIndex] = useState(null);
    ...
    const maxLength = useMemo(() => (filtredOptions === null || filtredOptions === void 0 ? void 0 : filtredOptions.length) - 1 || 0, [filtredOptions]);
    // useCallback
    ...
    const handleSelectCallback = useCallback((option, name) => () => {
        const { label, value } = option;
    ...
    const handlePressArrow = useCallback((event) => {
        if (isOpenList) {
    ...
    return (React__default.createElement("div", { className: `uiKit-wrapper ${className}` },
        label.length > 0 && (React__default.createElement("div", { className: "uiKit-select-label" },
            React__default.createElement(Typography$1, { value: label, type: TypeTypographyEnum.small, weight: WeightTypographyEnum.bold }))),
        React__default.createElement("div", { role: "button", tabIndex: -1, ref: ref, className: "uiKit-select-wrapper", onClick: handlePressEnter, onKeyDown: undefined },
            React__default.createElement(Input, { htmlFor: htmlFor, name: name, className: "uiKit-select-input", 
                ...
                ref: inputRef, hasError: hasError, disabled: disabled, value: searchValue, handleChange: handleFilter, placeholder: placeholder, onBlur: handleClose, onPressEnter: handlePressEnter, onPressEsc: handleClose, onPressArrow: handlePressArrow, icon: (isOpenList
                    && TypeSimpleIconEnum.angleUp)
                    || TypeSimpleIconEnum.angleDown }),
...
const Radio = (props) => {
    ...
    // Memo
    const memoClassName = useMemo(() => `uiKit-Radio${(hasError && '_hasError') || ''} ${className}`, [hasError, className]);
...
function RadioGroup({ name, label, options, onChange, onChangeField, checked, className = '', }) {
    const [selected, changeSelected] = useState(`${checked}`);
...
})(TooltipClassEnum || (TooltipClassEnum = {}));
var TooltipTypeEnum;
...
const Tooltip = (props) => {
    const { value, type = TooltipTypeEnum.bottomLeft, className = '' } = props;
    // useMemo
    const tooltipArrowClassName = useMemo(() => `${TooltipClassEnum[type]}`, [type]);
    const memoClassName = useMemo(() => `uiKit-tooltipWrapper ${className}`, [className]);
    return (React__default.createElement("div", { className: memoClassName },
...
const PagingItem = (props) => {
    ...
    const memoClassName = useMemo(() => `uiKit-pagging-paggingItem ${(active && 'uiKit-pagging-paggingItem-active') || ''}`, [active]);
...
const PagingRow = (props) => {
    ...
    const memoStylesOverflow = useMemo(() => {
        const width = 32;
        const padding = 16;
        let value = 0;
    ...
    const memoStylesWrapper = useMemo(() => {
...
const Paging = (props) => {
    ...
    // State
    const [currentPagePagging, setCurrentPage] = useState(initialCurrentPage);
    // Memo
    const memoClassName = useMemo(() => `uiKit-paging-wrapper ${className}`, [className]);
    // Callback
    ...
    const isActive = useCallback((value) => value === currentPagePagging, [currentPagePagging]);
    return (React__default.createElement("div", { className: memoClassName },
...
})(TypeIllustrationEnum || (TypeIllustrationEnum = {}));
...
const Illustrations = (props) => {
    ...
    // Memo
    const memoClassName = useMemo(() => `uiKit-illustration ${TypeIllustrationClassEnum[type]} ${className}`, [type]);
...
const Circle = (props) => {
    const { size = CircleSizeEnum.regular, color = CircleColorEnum.black, className = '' } = props;
    // Memo
    const memoClassName = useMemo(() => `${CircleClassEnum[size]}-${color} ${className}`, [size, color, className]);
    const memoSrc = useMemo(() => (color === CircleColorEnum.black && blackCircle) || yellowCircle, [color]);
...
})(SizeCardEnum || (SizeCardEnum = {}));

const Card = (props) => {
    const { children, size = SizeCardEnum.regular, className = '', handleClose } = props;
    // useState
    const [isVisible, toggleVisible] = useState(true);
    // useMemo
    const memoClassName = useMemo(() => `${TypeCardClassEnum[size]} ${className}`, [size, className]);
    // useCallback
...
})(TypeMulticolorEnum || (TypeMulticolorEnum = {}));
...
const Multicolor = (props) => {
    const { type, width = 'auto', height = 'auto', className = '' } = props;
    // Memo
    const memoClassName = useMemo(() => `uiKit-multicolor ${TypeMulticolorClassEnum[type]} ${className}`, [type, className]);
    const styles = useMemo(() => ({
...
const Badge = (props) => {
    ...
    // Memo
    const memoValue = useMemo(() => (value <= MAX_LIMIT_BADGE && `${value}`) || '99+', [value]);
    const memoClassName = useMemo(() => `uiKit-badge ${className}`, [className]);
    return (React__default.createElement("div", { className: memoClassName },
...
const ModalLayer = (props) => {
    ...
    // Memo
    const memoClassName = useMemo(() => `uiKit-modalLayer ${className}`, [className]);
    // Callback
...
function Manager(_ref) {
  ...
  var hasUnmounted = useRef(false);
  useEffect(function () {
    return function () {
      hasUnmounted.current = true;
    };
  }, []);
  var handleSetReferenceNode = useCallback(function (node) {
...
const InputDynamic = forwardRef((_a, ref) => {
    ...
    return createElement(tag, Object.assign({ autoComplete,
        ref,
...
const InputBase = forwardRef((_a, ref) => {
    var { mark, className = '', icon, type = InputDynamicType.text, iconSide = IconSideEnum$1.right, onIconClick } = _a, rest = __rest(_a, ["mark", "className", "icon", "type", "iconSide", "onIconClick"]);
...
function DatePicker({ disabled, htmlFor = true, hasError, startDate, endDate, name, nameEnd, label, className = '', hasRange, hint, error, hasDirty, minDate, onChange, onChangeField, onChangePeriod, }) {
    const handleChangePeriod = useCallback((dates) => {
    ...
    const changeHandler = useMemo(() => (hasRange && handleChangePeriod) || handleChange, [handleChange, handleChangePeriod, hasRange]);
...
const InputMasked = IMaskMixin((_a) => {
...
const Input$1 = (_a) => {
    ...
    const changeField = useCallback((value, mask) => {
...
function Dropdown$1(_a) {
    ...
    // выбор опции через клик
    const handleSelectCallback = useCallback((option) => {
        const { label, value } = option;
        setSearch(label);
        onChange === null || onChange === void 0 ? void 0 : onChange(value);
        onChangeField === null || onChangeField === void 0 ? void 0 : onChangeField(value, name);
...
export { AutoCloseNitificationType, AutoCloseNotification, Badge, Button, Card, Checkbox$1 as Checkbox, Checkbox as CheckboxDeprecated, Circle, CircleClassEnum, CircleColorEnum, CircleSizeEnum, DDMMYYYY, DatePicker, Dots, DotsSizeEnum, Dropdown$1 as Dropdown, Dropdown as DropdownDeprecated, FormField, IconSideEnum$1 as IconSideEnum, Illustrations, Input$1 as Input, InputBase, Input as InputDeprecated, InputNumber, InputPassword, LabeSizeEnum, Label, LabelTypeEnum, Link, ModalLayer, Multicolor, Notification, NotificationSizeEnum, NotificationTypeEnum, NotificationsContainer, Paging, Radio$1 as Radio, Radio as RadioDeprecated, RadioGroup$1 as RadioGroup, RadioGroup as RadioGroupDeprecated, SimpleIcon, SizeButtonEnum, SizeCardEnum, Textarea$1 as Textarea, Textarea as TextareaDeprecated, Tooltip, TooltipClassEnum, TooltipTypeEnum, TypeButtonEnum, TypeCheckboxEnum, TypeIllustrationEnum, TypeLinkEnum, TypeMulticolorEnum, TypeSimpleIconEnum, TypeTypographyEnum as TypeTypographyDeprecatedEnum, Typography, Typography$1 as TypographyDeprecated, TypographyFont, TypographyFontWeight, TypographyTag, TypographyType, WeightTypographyEnum as WeightTypographyDeprecatedEnum, YYYYMMDDHHmmss, dayjs_min as dayjs, safeParse, toast };
